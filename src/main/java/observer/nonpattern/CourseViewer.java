...
public class CourseViewer extends JFrame implements ActionListener,
	...
	public CourseViewer() {
		...
		buttonMode = new JButton("Pie Chart");
		...
		constraints.weighty = 0.0;
		constraints.gridy = 2;
		sliderPanel.add(buttonMode, constraints);
	...
	public void paint(Graphics g) {
		System.out.println(buttonMode.getText());
		super.paint(g);
		if(buttonMode.getText().contentEquals("Bar Chart")) {
			System.out.println(buttonMode.getText());
			...
			Rectangle area = scrollPane.getVisibleRect();
			double total = 0.0D;
		      ...
		      for (int i = 0; i < sliders.size(); i++) {
		         ...
		         int arcAngle = (int) (sliders.get(i).getValue() * 360 / total);
		         g.setColor(LayoutConstants.courseColours[i]);
		         g.fillArc(LayoutConstants.xOffset, LayoutConstants.yOffset+LayoutConstants.graphHeight/3, LayoutConstants.graphHeight, LayoutConstants.graphHeight, startAngle, arcAngle);
		         
		         curValue += sliders.get(i).getValue();
		      }
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource().equals(buttonMode)) {
			if(buttonMode.getText().equals("Bar Chart") ) {
				buttonMode.setText("Pie Chart");
			} else {
				buttonMode.setText("Bar Chart");
			}
			this.repaint();
			this.validate();
		}
